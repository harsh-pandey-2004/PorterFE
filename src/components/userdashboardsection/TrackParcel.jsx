...
const TrackParcels = () => {
  ...
  const getParcelStatus = (trackingNumber) => {
    const mockStatuses = {
      '12345': [
        { step: 'Order Placed', status: 'Completed' },
        { step: 'Picked Up', status: 'Completed' },
        { step: 'In Transit', status: 'Completed' },
        { step: 'Out for Delivery', status: 'Ongoing' },
        { step: 'Delivered', status: 'Pending' },
      ],
      '67890': [
        { step: 'Order Placed', status: 'Completed' },
        { step: 'Picked Up', status: 'Completed' },
        { step: 'Delivered', status: 'Completed' },
      ],
    };

    return mockStatuses[trackingNumber] || null;
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    const status = getParcelStatus(trackingNumber);
    if (status) {
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-12 h-full">
      <h2 className="text-4xl md:text-5xl font-extrabold text-center text-[#1D3557] mb-12">Track Your Parcel</h2>
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-6">
                ...
                <input
                  ...
                  required
                  className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-[#4FC3F7] transition-shadow"
                  placeholder="Tracking Number"
                />
              </div>
              <button
                type="submit"
                className="w-full bg-[#1D3557] text-white font-semibold py-3 rounded-lg hover:bg-[#4FC3F7] transition-all duration-300 ease-in-out hover:shadow-lg focus:shadow-lg"
              >
                Track Parcel
              </button>
        ...
        <div className="w-full md:w-1/2">
          {parcelStatus ? (
            <div className="bg-white rounded-lg shadow-lg p-6 md:p-8">
              <h3 className="text-2xl font-bold mb-6 text-[#1D3557]">Tracking Information</h3>
