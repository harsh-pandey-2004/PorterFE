...
const TrackParcels = () => {
  ...
  const generateStatusHistory = (currentStatus) => {
    ...
    return statusOrder.map((status, index) => ({
      step: status.replace(/_/g, ' ').toUpperCase(), // Format for display
      status:
        index < currentIndex
          ? 'Completed'
          : index === currentIndex
  ...
  const getParcelStatus = async (trackingNumber) => {
    try {
      const response = await axios.get(
        `https://porterbe.onrender.com/api/parcels/${trackingNumber}/track`
      );
      // Assume the API returns a single status (e.g., "picked_up")
